...
function useLocale() {
  const [locale, setLocale] = useState<Locale>(() =>
    localStorage.getItem("lifeos.locale") === "ar" ? "ar" : "en",
  );
  const set = (next: Locale) => {
    localStorage.setItem("lifeos.locale", next);
  ...
  return (
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <nav>
          <a href="#home">{intl.formatMessage({ id: "navigation.home" })}</a>
...
function AreaScreen() {
  ...
  const create = useMutation({
    mutationFn: (title: string) =>
      unwrapCommand<ActionReceipt<Area>>(
        commands.createArea({ title, operationId: operationId() }),
      ),
    onSuccess: (receipt) => {
      form.reset();
  ...
  const undoMutation = useMutation({
    mutationFn: (undoBatchId: string) =>
      unwrapCommand(
        commands.undoAction({ undoBatchId, operationId: operationId() }),
      ),
    onSuccess: () => {
      setUndo(null);
      void qc.invalidateQueries({ queryKey: ["areas"] });
  ...
  return (
    <section className="canvas">
      ...
      <div className="page-heading">
        <div>
          <h1>{intl.formatMessage({ id: "area.title" })}</h1>
          <p>{intl.formatMessage({ id: "area.description" })}</p>
        </div>
        <span className="metric">{areas.data?.length ?? 0}</span>
      </div>
      <form
...
function DialogHost({
  ...
  const key = `dialog.${dialog}` as keyof typeof messages.en;
  return (
