...
- Completed — M3 Project foundation increment: migration 6, typed Core/Tauri Project create/list contracts, active-parent validation, canonical SQLite persistence, FTS projection, event/audit/version/undo records, and a localized Projects route now work. Safe movement, cycle checks for existing trees, lifecycle controls, relations, details, and shared views remain pending.
...
- In progress — M3 Projects, tasks, goals, and relations: Goal foundation/lifecycle and Project creation/nesting foundation are delivered; Goal completion/relations/detail, Project moves/lifecycle/details, Tasks, milestones, RelationGraph, and reusable views remain pending.
...
- M3 Project foundation verification: generated contract drift check, TypeScript type checking, linting, and Prettier formatting passed. JavaScript tests: 13 desktop tests passed; contracts/i18n intentionally have no test files. Rust workspace tests: 28 passed, 0 failed, including Project parent validation, child persistence across restart, and typed undo. The renderer build completed with the existing non-failing chunk-size warning: 510.84 kB minified/159.05 kB gzip. `CI=true pnpm tauri build` produced a fresh macOS ARM64 bundle at 2026-08-12 12:19 local host time; strict deep code-sign and `hdiutil verify` passed and its SHA-256 is `0c6e0c7f8fa910f065e35a554263f502d1c8fb90463ed01b1a1934e97588b8db`.
