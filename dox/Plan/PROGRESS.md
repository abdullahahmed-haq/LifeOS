...
- In progress — F0 Foundation completion: added append-only migration 2 for canonical settings, policy/job/cursor persistence foundations, typed settings IPC, first-paint locale/theme bootstrap, a real General Settings screen, typed routes, and a refactored Area route. Native credential storage, generic lifecycle safety, full route registry, and Windows evidence remain pending.
